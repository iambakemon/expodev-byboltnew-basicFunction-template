...
export default function WifiScreen() {
  const [wifiState, setWifiState] = useState<string>('');
  ...
  const checkWifiStatus = async () => {
    try {
      ...
      setWifiState(networkState.type);
    } catch (error) {
      console.error('WiFiステータスの確認エラー:', error);
    }
  };

  const handleConnect = () => {
    // ここでESP32へのWiFi接続処理を実装
  };
